...
export const isMachinePreview = (value: string): boolean =>
    MACHINE_PREVIEW_PATTERN.test(normalizeText(value));

export const getTargetSource = (toolCall: IAiToolCall, fallbackTarget: string): string => {
...
export const getTargetLeafLabel = (
...
export const getAggregateDurationLabel = (durations: Array<number | undefined>): string | null => {
    const safeDurations = durations.filter((value): value is number =>
    ...
    return formatElapsed(safeDurations.reduce((sum, value) => sum + value, 0));
};