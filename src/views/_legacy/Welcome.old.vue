<!--
  旧欢迎页保留说明：
  - 保留原因：欢迎页替换后的一个 release 内用于回滚与视觉对比
  - 复活条件：新欢迎页出现阻塞发布的严重回归
  - 负责人：@xiaojianc
  - 截止日期：2026-06-30
-->
<template>
    ...
    >
      ...
      <div class="splash-progress-wrap">
        <div
          ...
          :class="{ 'is-error': Boolean(error) }"
          role="progressbar"
          :aria-valuenow="Math.round(roundedProgress)"
          aria-valuemin="0"
          aria-valuemax="100"
        >
...
const resolveAutoplayProgress = (elapsed: number): number => {
  ...
  return lerp(
    ...
    easeOutCubic(
      clamp01(
        (ratio - SHORT_PROGRESS_SECOND_PHASE_RATIO) / (1 - SHORT_PROGRESS_SECOND_PHASE_RATIO),
      ),
    ),
  );
};
...
const updateProgress = (now: number): void => {
  ...
  if (remainingProgress > 0) {
    ...
    progress.value = Math.min(
      progressTarget.value,
      progress.value +
        Math.max(isFinishSprint ? 0.32 : 0.06, remainingProgress * (isFinishSprint ? 0.2 : 0.1)),
    );
  }
