...
mod tests {
    ...
    fn run_script_request_validate_rejects_blank_and_bad_size() {
        let valid = WslLinkTerminalRunScriptRequest {
        ...
        let blank = WslLinkTerminalRunScriptRequest {
            run_id: "  ".to_string(),
            ..valid.clone()
        };
        assert!(blank.validate().is_err());

        let bad_size = WslLinkTerminalRunScriptRequest {
            cols: 1,
            ..valid.clone()
        };
    ...
    fn open_interactive_request_validate_checks_fields_and_size() {
        let valid = WslLinkTerminalOpenInteractiveRequest {
        ...
        assert!(valid.validate().is_ok());
        assert!(WslLinkTerminalOpenInteractiveRequest {
