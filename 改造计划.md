...
**现状**：Rust `ai/gateway/suggestions.rs` + 前端 `useAiSuggestionPool` + `AiFloatingSuggestions` + `ai-elements/suggestion/` + `utils/ai-suggestion-selection.ts` + `constants/ai-suggestions.ts` + `utils/suggestion-layout.ts`。无 schema。
