...
// 对比
if (!pkgVersion) {
    results.push({ severity: 'ERROR', message: 'package.json 中未找到 version 字段', file: 'package.json' });
} else {
    results.push({ severity: 'PASS', message: `package.json version = ${pkgVersion}`, file: 'package.json' });
}

if (!tauriVersion) {
    results.push({ severity: 'WARN', message: 'src-tauri/tauri.conf.json 未找到 version 字段（可能正常）', file: 'src-tauri/tauri.conf.json' });
    ...
    results.push({ severity: 'PASS', message: `tauri.conf.json version = ${tauriVersion} ✓`, file: 'src-tauri/tauri.conf.json' });
}

if (!cargoVersion) {
    results.push({ severity: 'WARN', message: 'src-tauri/Cargo.toml 未找到 version 字段', file: 'src-tauri/Cargo.toml' });
    ...
    results.push({ severity: 'PASS', message: `Cargo.toml version = ${cargoVersion} ✓`, file: 'src-tauri/Cargo.toml' });
}
