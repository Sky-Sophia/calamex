...
const normalizeInsertValue = (value: unknown): { insertText?: string; insertAsSnippet?: boolean } => {
...
const toOptionArgumentFields = (argumentList: ArgumentSpec[]): { arg?: ArgumentSpec; args?: ArgumentSpec[] } => {
...
const mergeValueSuggestionSpec = (baseSuggestionSpec: ValueSuggestionSpec, overrideSuggestionSpec: ValueSuggestionSpec): ValueSuggestionSpec =>
...
const mergeValueSuggestionList = (baseSuggestions: ValueSuggestionSpec[], overrideSuggestions: ValueSuggestionSpec[]): ValueSuggestionSpec[] => {
...
const mergeArgumentSpec = (baseArgumentSpec: ArgumentSpec, overrideArgumentSpec: ArgumentSpec): ArgumentSpec =>
    pruneArgumentSpec({
...
const mergeArgumentList = (baseArguments: ArgumentSpec[], overrideArguments: ArgumentSpec[]): ArgumentSpec[] => {
...
const mergeOptionSpec = (baseOptionSpec: OptionSpec, overrideOptionSpec: OptionSpec): OptionSpec => {
...
const mergeOptionList = (baseOptions: OptionSpec[], overrideOptions: OptionSpec[]): OptionSpec[] => {
...
const mergeCommandNode = (baseCommandNode: CommandNode, overrideCommandNode: CommandNode): CommandNode =>
    pruneCommandNode({
...
const mergeCommandList = (baseCommands: CommandNode[], overrideCommands: CommandNode[]): CommandNode[] => {
...
const resolveLoadedSpec = async (specDefinition: unknown, loadSpecStack: Set<string> = new Set()): Promise<FigDefinition | null> => {
...
const transformCommand = async (commandDefinition: unknown, loadSpecStack: Set<string> = new Set()): Promise<CommandNode | null> => {
...
const writeGeneratedCatalog = async (generatedCatalog: CommandNode[]): Promise<{ updatedFileCount: number }> => {
