...
/** 声明为 dormant 的目录列表 */
const DORMANT_DIRS = [
    { dir: 'src/router', mainFile: 'index.ts' },
];
...
for (const { dir, mainFile } of DORMANT_DIRS) {
    ...
    if (!fs.existsSync(readmePath)) {
        const { exempt, expired, entry } = checkExemption(exemptions, `${relDir}/${mainFile}`, 'dormant-no-readme');
        if (exempt) {
            results.push({
                severity: 'WARN',
...
const SCAN_FOR_IMPORT: string[] = ['src/composables', 'src/views', 'src/layouts', 'src/store', 'src/services'];
