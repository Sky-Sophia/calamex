...
fn create_window<T: UserEvent, F: Fn(RawWindow) + Send + 'static>(
  ...
  if use_visual_hosting {
    ...
    if let Some(pending_webview) = webview.as_mut() {
      if pending_webview.webview_attributes.background_color.is_none() {
        pending_webview.webview_attributes.background_color = background_color.map(Into::into);
      }
    }
  }
...
"#,
  ...
  #[cfg(windows)]
  if webview_attributes.visual_hosting {
    webview_builder = webview_builder.with_visual_hosting(true);
  }
