...
pub struct WebViewAttributes<'a> {
  ...
  /// Whether WebView2 visual hosting should be used instead of the legacy child HWND hosting path.
  ///
  /// ## Platform-specific
  ///
  /// - **Windows**: Enables composition controller hosting when the `visual-hosting` feature is enabled.
...
pub trait WebViewBuilderExtWindows {
  ...
  /// Enables WebView2 visual hosting, replacing the legacy child HWND hosting path.
  ///
  /// This requires the `visual-hosting` cargo feature to be enabled.
  fn with_visual_hosting(self, enabled: bool) -> Self;
}
...
impl WebViewBuilderExtWindows for WebViewBuilder<'_> {
  ...
  fn with_visual_hosting(mut self, enabled: bool) -> Self {
    self.attrs.visual_hosting = enabled;
    self
  }
}
